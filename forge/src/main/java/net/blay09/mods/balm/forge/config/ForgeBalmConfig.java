...
public class ForgeBalmConfig extends AbstractBalmConfig {
    ...
    private static ForgeConfigSpec.ConfigValue<?> addPropertyToSpec(ConfiguredProperty<?> property, ForgeConfigSpec.Builder spec) {
        ...
        if (property instanceof ConfiguredBoolean configuredBoolean) {
            return spec.define(configuredBoolean.name(), configuredBoolean.defaultValue().booleanValue());
        } else if (property instanceof ConfiguredDouble configuredDouble) {
            return spec.define(configuredDouble.name(), configuredDouble.defaultValue());
        } else if (property instanceof ConfiguredEnum<?> configuredEnum) {
            return defineEnum(spec, configuredEnum);
        } else if (property instanceof ConfiguredFloat configuredFloat) {
            return spec.define(configuredFloat.name(), configuredFloat.defaultValue().doubleValue());
        } else if (property instanceof ConfiguredInt configuredInt) {
            return spec.define(configuredInt.name(), configuredInt.defaultValue());
        } else if (property instanceof ConfiguredList<?> configuredList) {
            return spec.defineListAllowEmpty(configuredList.name(),
                    mapConfigCollectionToNeoForge(configuredList.defaultValue()),
                    (it) -> validateListElement(configuredList, it));
        } else if (property instanceof ConfiguredLong configuredLong) {
            return spec.define(configuredLong.name(), configuredLong.defaultValue());
        } else if (property instanceof ConfiguredResourceLocation configuredResourceLocation) {
            return spec.define(configuredResourceLocation.name(), configuredResourceLocation.defaultValue().toString());
        } else if (property instanceof ConfiguredSet<?> configuredSet) {
            return spec.defineListAllowEmpty(configuredSet.name(),
                    mapConfigCollectionToNeoForge(configuredSet.defaultValue()),
                    (it) -> validateSetElement(configuredSet, it));
        } else if (property instanceof ConfiguredString configuredString) {
            return spec.define(configuredString.name(), configuredString.defaultValue());
        } else {
    ...
    public static Object mapConfigValueToNeoForge(Object value) {
        if (value instanceof ResourceLocation resourceLocation) {
            ...
            return mapConfigCollectionToNeoForge(set);
        } else if (value instanceof List<?> list) {
            return mapConfigCollectionToNeoForge(list);
        } else {
            return value;
        }
    }
    ...
    public static Object mapConfigValueFromNeoForge(ConfiguredProperty<?> property, Object value) {
        ...
        } else if (property instanceof ConfiguredFloat) {
            return ((Double) value).floatValue();
        } else if (property instanceof ConfiguredList<?> listProperty) {
            return mapConfigListFromNeoForge(listProperty, (List<?>) value);
        } else if (property instanceof ConfiguredSet<?> setProperty) {
            return mapConfigSetFromNeoForge(setProperty, (List<?>) value);
        } else {
            return value;
        }
    }
    ...
    public void registerConfig(BalmConfigSchema schema) {
        ...
        eventBus.addListener((ModConfigEvent.Loading event) -> {
            ...
            final var identifier = new ResourceLocation(modConfig.getModId(), modConfig.getType().extension());
